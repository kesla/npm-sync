...
import setupInstallPackage from './install-package';

const log = process.env.NPM_SYNC_LOGGING ?
  console.log : () => {};

const setupInstall = installPackage => async function install({tree = {}, dir}) {
...
export default downloadPackage => {
  const install = setupInstall(setupInstallPackage(downloadPackage));
